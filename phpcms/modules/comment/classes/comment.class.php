	...
	public function add($commentid, $siteid, $data, $id = '', $title = '', $url = '') {
		...
		if ($id) {
			...
			if ($r) {
				...
				if ($r['reply']) {
					$data['content'] = '<div class="box-reply">'.str_replace('<span></span>', '<span class="blue f12">'.$r['username'].' '.L('chez').' '.format::date($r['creat_at'], 1).L('release').'</span>', $r['content']).'</div>'.$data['content'];
				} else {
					$data['content'] = '<div class="box-reply"><span class="blue ">'.$r['username'].' '.L('chez').' '.format::date($r['creat_at'], 1).L('release').'</span><pre>'.$r['content'].'</pre></div>'.$data['content'];
				}
				$data['reply'] = 1;
			}
		}
