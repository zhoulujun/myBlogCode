...
class index {
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
				...
				if($modelid) {
					...
					if(!empty($segment_q)) {
						...
						foreach($replace as $replace_arr_v) {
							$replace_arr[] =  '<span class="red">'.$replace_arr_v.'</span>';
						}
						...
						foreach($data as $_k=>$_v) {
							$data[$_k]['title'] = str_replace($q, '<span class="red">'.$q.'</span>', $_v['title']);
							$data[$_k]['description'] = str_replace($q, '<font class="red">'.$q.'</font>', $_v['description']);
						}
					}
				} else {
					//读取专辑搜索接口
            ...
            $dataSearchList = isset($data) ? $data : '';
